...
func TestReturnsError(t *testing.T) {
	...
	f1 := func(ctx context.Context) (string, string, error) {
		callCount1++
		fmt.Println("Init transaction ")
		return "hello", "hello", errors.New("some error")
	}
	f2 := func(ctx context.Context, s string, s2 string) error {
		callCount2++
	...
	context.WithValue(ctx, "s", "test")

	c := NewCoordinator(ctx, context.Background(), s, New())
	err := c.Play()
