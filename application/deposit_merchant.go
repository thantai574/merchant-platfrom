...
func (us *OrderApplication) DepositMerchant(ctx context.Context, request *order_system.DepositMerchantRequest) (order_dto *entities.OrderEntity, err error) {
	sg := saga.NewSaga("Deposit Merchant")

	err = sg.AddStep(&saga.Step{
		Name: "INIT_ORDER",
		Func: func(c context.Context) (err error) {
			order_dto, err = us.InitOrder(ctx, &entities.OrderEntity{
				ServiceID:    constants.TRANSTYPE_WALLET_CASH_IN_MC,
				ServiceType:  constants.SERVICE_TYPE_WALLET,
				OrderType:    constants.TRANSTYPE_WALLET_CASH_IN_MC,
	...
	err = sg.AddStep(&saga.Step{
		...
		Func: func(c context.Context) (err error) {
			dto := &service_transaction.ETransactionDTO{
				TransactionType:    constants.TRANSTYPE_WALLET_CASH_IN_MC,
				ServiceType:        constants.SERVICE_TYPE_WALLET,
				Amount:             request.OrderRequest.Amount,
				...
				MerchantTypeWallet: request.MerchantAccountType,
