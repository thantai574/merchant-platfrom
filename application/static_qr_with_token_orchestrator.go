...
func (us *OrderApplication) StaticQRWithToken(ctx context.Context, request *order_system.PaymentMerchantRequest, response *order_system.PaymentMerchantResponse) (orderDto *entities.OrderEntity, err error) {
	sg := saga.NewSaga("StaticQRWithToken With Token ")
	...
	if request.ConfirmPaymentTokenRequest.Otp != "" {
		...
		bankTraceId = orderDto.BankTransactionId
	...
	err = sg.AddStep(&saga.Step{
		...
		Func: func(c context.Context) (err error) {
			...
			trans.BankTransactionId = bankTraceId
			trans.ProviderMerchantID = request.OrderRequest.MerchantID
			trans.MerchantID = request.OrderRequest.MerchantID
